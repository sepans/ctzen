...
const Home: React.FC<HomeProps> = ({ me: {nextQuestion, user} }) => {
  ...
  const questionButton = nextQuestion ? <Link to={`/question/${nextQuestion.id}`}>
...
const ButtonBox = ({ children }) => <Box justifyContent={"center"} flex={1} display="flex" alignItems={"center"}>{children}</Box>
